...
(transcript: [`vivado_sim_1886us.log`](vivado_sim_1886us.log)):
...
| ML-DSA-65 verification | 109,145 | 1,091 µs | 1.5-2.5 ms |
| **Total authentication latency** | **188,554** | **1.89 ms** | ~3.35 ms |

Testbench result: **47 PASS**, `TB: PASS - ALL TEST CASES PASSED`.

**2.6x faster** than the 4.94 ms this design started at, and 1.8x faster than
the paper figure.

| Configuration | ML-DSA verify | Total unlock |
|-------|---------------|--------------|
| Before this work | 3,329 µs | 4.94 ms |
| Original design (companion folder) | 2,554 µs | 3.35 ms |
| **This design** | **1,091 µs** | **1.89 ms** |

### Where the speed came from

| Block | Before | After |
|-------|--------|-------|
| ML-DSA NTT / INTT | ~10.7 cyc/butterfly | 2 cyc/butterfly, pipelined |
| ML-DSA ExpandA rejection sampling | 4 cyc/candidate | 1 cyc/candidate |
| ML-KEM NTT / INTT | 7 cyc/butterfly | 2 cyc/butterfly, pipelined |
| ML-KEM basemul / poly add / compress | 5-13 cyc/coefficient | 1 cyc/coefficient |
| ML-KEM ByteEncode / ByteDecode | 4-6 cyc/byte | ~1 cyc/byte |

ML-KEM encapsulation as a whole went from 131,652 to 49,656 cycles.

---

## This Design

This folder is the **optimized design**: the NTT pipelines run at full rate and
the full authentication takes 1.89 ms. A companion folder, `original_design`,
holds the same RTL at the ~3.35 ms reported in the paper.

Both folders contain the same RTL and produce bit-identical results; they are
configured for different verification speeds.
