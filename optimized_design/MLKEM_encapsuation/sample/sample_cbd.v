...
module sample_cbd #(
    ...
    // Implemented with continuous assignments so Icarus elaborates it
    // correctly (a function+memory lookup mis-elaborated at offset 0).
    wire [7:0] cb_b0 = prf_buf[bit_offset[10:3]];
    wire [7:0] cb_b1 = prf_buf[bit_offset[10:3] + 1];
    wire [15:0] cb_two = {cb_b1, cb_b0};
    assign extracted_bits = (cb_two >> bit_offset[2:0]) & ((1 << BITS_PER_COEFF) - 1);
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_COLLECT: begin
                    // Level 'ready': keep the producer advancing one byte per
                    // cycle. Capturing every cycle we see (valid) then samples
                    // each presented byte exactly once (data advances on the
                    // same edge, sampled pre-edge with the 1-cycle lag).
                    prf_req <= 1'b1;
                    if (prf_valid) begin
