...
for line in open(r"pkg\ntt_rom.v", encoding="utf-8"):
    ...
    if m:
        rom[int(m.group(1))] = -int(m.group(3)) if m.group(2).startswith("-") else int(m.group(3))

diffs = [(i, rom[i], real[i]) for i in range(128) if rom[i] != real[i]]
print("total diffs:", len(diffs))
for i, r, t in diffs[:40]:
    print(f"  [{i}] rom={r}  real={t}")