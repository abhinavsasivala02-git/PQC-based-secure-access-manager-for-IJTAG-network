...
module kpke_encrypt #(
    ...
    mlkem_poly_ram #(.DEPTH(768),.ADDR_W(10)) u_ram0 (.clk(clk),.a_wen(ram0_wen),.a_addr(ram0_addr_a),.a_wdata(ram0_wdata),.a_rdata(ram0_rdata_a),.b_addr(ram0_addr_b),.b_rdata(ram0_rdata_b));
    mlkem_poly_ram #(.DEPTH(768),.ADDR_W(10)) u_ram1 (.clk(clk),.a_wen(ram1_wen),.a_addr(ram1_addr_a),.a_wdata(ram1_wdata),.a_rdata(ram1_rdata_a),.b_addr(ram1_addr_b),.b_rdata(ram1_rdata_b));
    mlkem_poly_ram u_ram2 (.clk(clk),.a_wen(ram2_wen),.a_addr(ram2_addr_a),.a_wdata(ram2_wdata),.a_rdata(ram2_rdata_a),.b_addr(ram2_addr_b),.b_rdata(ram2_rdata_b));
    mlkem_poly_ram u_ram3 (.clk(clk),.a_wen(ram3_wen),.a_addr(ram3_addr_a),.a_wdata(ram3_wdata),.a_rdata(ram3_rdata_a),.b_addr(ram3_addr_b),.b_rdata(ram3_rdata_b));
    mlkem_poly_ram u_ram4 (.clk(clk),.a_wen(ram4_wen),.a_addr(ram4_addr_a),.a_wdata(ram4_wdata),.a_rdata(ram4_rdata_a),.b_addr(ram4_addr_b),.b_rdata(ram4_rdata_b));
    mlkem_poly_ram u_ram5 (.clk(clk),.a_wen(ram5_wen),.a_addr(ram5_addr_a),.a_wdata(ram5_wdata),.a_rdata(ram5_rdata_a),.b_addr(ram5_addr_b),.b_rdata(ram5_rdata_b));
    ...
    sample_ntt u_sample_ntt (
        ...
        .kec_req      (sntt_kec_req),
        .kec_din      (sntt_kec_din),
        .kec_busy     (sntt_kec_busy),
        .kec_done     (sntt_kec_done),
        .kec_dout     (sntt_kec_dout),
    ...
    shake256 u_prf (
        ...
        .kec_req      (prf_kec_req),
        .kec_din      (prf_kec_din),
        .kec_busy     (prf_kec_busy),
        .kec_done     (prf_kec_done),
        .kec_dout     (prf_kec_dout),
    ...
    reg bmul_use_t;
    // a read on port B, leaving port A for the product write-back
    assign bmul_a_rd = bmul_use_t ? ram1_rdata_b : ram3_rdata_b;
    ...
    reg arith_use_v;
    // accumulator read on port B, leaving port A for the write-back
    assign arith_a_rd = arith_use_v ? ram4_rdata_b : ram2_rdata_b;
